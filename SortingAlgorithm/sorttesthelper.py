...
import copy
from SortingAlgorithm import selectionsort,insertionsort,bubblesort,shellsort,mergesort,quicksort,heapsort,mergesortBU,quicksort3way
...
class SortTestHelper(object):
    ...
    def testSort(self, str, arr):
        ...
        if str == "Quick Sort 3 Way" :
            copy_items = copy.deepcopy(arr)
            print("-" * 10 + str + "-" * 10)
            qsort = quicksort3way.QuickSort3Way()
            length = len(copy_items)
            start = timer()
            sorted_items = qsort.sort(copy_items, 0, length-1)
            end = timer()
            ...
            print(sorted_items)
            if self.isSorted(sorted_items):
                print("python quick sort 3 way - %fs" % duration)
        ...
        if str == "Merge Sort Bottom Up":
            copy_items = copy.deepcopy(arr)
            print("-" * 10 + str + "-" * 10)
            msort = mergesortBU.MergeSortBU
            start = timer()
            sorted_items = msort.sort(copy_items)
            end = timer()
            duration = end - start
